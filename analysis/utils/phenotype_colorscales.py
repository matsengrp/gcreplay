...
def main():
    ...
    sns.histplot(pd.read_csv(dms, index_col="mutation", dtype=dict(position_IMGT=pd.Int16Dtype())).delta_bind_CGG, label="DMS", ax=ax1)
    sns.histplot(pd.Series([node.delta_bind for tree in trees for node in tree.tree.traverse() if not np.isnan(node.delta_bind)]), label="trees", zorder=0, ax=ax1)
    ax1.legend()

    sns.histplot(pd.read_csv(dms, index_col="mutation", dtype=dict(position_IMGT=pd.Int16Dtype())).delta_expr, label="DMS", ax=ax2)
    sns.histplot(pd.Series([node.delta_expr for tree in trees for node in tree.tree.traverse() if not np.isnan(node.delta_expr)]), label="trees", zorder=0, ax=ax2)
    ax2.legend()

    sns.histplot(pd.read_csv(dms, index_col="mutation", dtype=dict(position_IMGT=pd.Int16Dtype())).delta_psr, label="DMS", ax=ax3)
    sns.histplot(pd.Series([node.delta_psr for tree in trees for node in tree.tree.traverse() if not np.isnan(node.delta_psr)]), label="trees", zorder=0, ax=ax3)
    ax3.legend()

...
def wrangle_annotation(igh_airr, igk_airr, input_fasta, key_file, output):
    """
...
@click.option("-n", "--add-naive", type=click.BOOL, default=True)
...
def df_groupby(dataframe, columns, sample, output_prefix):
    ...
    for group, groupdf in df.groupby(list(columns)):
        ...
        if sample != None:
            ...
            ).to_csv(f"{output_prefix}-{group_string}.csv", index=False)
        else:
            groupdf.sort_values(by="ID_HK").to_csv(
                f"{output_prefix}-{group_string}.csv", index=False
            )
...
@click.option("-rid", "--render-idlabel", type=click.BOOL, default=True)
def node_featurize(
    gctree_file,
    idmapfile,
    variant_scores,
    ...
    for node in tree.tree.traverse():
        ...
        igk_nt_mutations = nt_mutations(
            naive_igk_nt.upper(), igk_nt.upper(), site_idx_offset=igk_idx
        )
        ...
        if len(node.isotype) == 1:
            ...
            isotype = ",".join(f"{key}:{node.isotype[key]}" for key in node.isotype)
        ...
        node_features["sampled_cell_ids"].append(
        ...
        node_features["descendant_abundance"].append(
        ...
        for i, phenotype in enumerate(["delta_bind_CGG", "delta_expr"]):
            ...
            if final_variant_scores is not None:
                ...
                additive_score = (
                    np.nan
                    if (igh_has_stop or igk_has_stop)
    ...
    phenotypes = ["delta_bind", "delta_expr"]
...
def featurize_seqs(
    hk_df, igh_frame, igk_frame, igk_idx, variant_scores, naive_sites, output
):
        ...
        if ct == "GC":
            ...
            for rc in glob.glob(f"{gct_out}/*"):
                if os.path.isdir(rc):
                    ...
                    node_data.insert(
                        0, "ranking_coeff_strategy", ranking_coeff_strategy
                    )
